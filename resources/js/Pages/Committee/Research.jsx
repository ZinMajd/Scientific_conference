...
export default function CommitteeResearch() {
    ...
    useEffect(() => {
        ...
        } else if (location.pathname.endsWith('/recommend')) {
    ...
    const getStatusLabel = (status) => {
        switch(status) {
            ...
            case 'production_revision_required': return { text: 'تعديل إنتاجي مطلوب', color: 'bg-orange-50 text-orange-600', dot: 'bg-orange-600' };
            case 'in_production': return { text: 'قيد الإنتاج', color: 'bg-blue-50 text-blue-600', dot: 'bg-blue-600' };
            ...
            case 'scheduled': return { text: 'تمت الجدولة', color: 'bg-teal-50 text-teal-600', dot: 'bg-teal-600' };
            case 'published': return { text: 'منشور', color: 'bg-sky-50 text-sky-600', dot: 'bg-sky-600' };
                                                ...
                                                {paper.author_id === user.id ? (
                                                    ...
                                                    <>
                                                        ...
                                                        {paper.status === 'accepted' && (
                                                            <button 
                                                                onClick={async () => {
                                                                    if(confirm('هل أنت متأكد من تحويل هذا البحث لمكتب الإنتاج؟')) {
                                                                        try {
                                                                            await axios.post(`/api/production/papers/${paper.id}/send`);
                                                                            alert('تم التحويل للإنتاج');
                                                                            fetchPapers();
                                                                        } catch(e) {
                                                                            alert('فشل التحويل');
                                                                        }
                                                                ...
                                                                className="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 transition shadow-sm" 
                                                                title="تحويل للإنتاج"
                                                            >
                                                                🏗️
                                                            </button>
                    ...
                    <div className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl">
                        ...
                        <form onSubmit={submitAssignment} className="space-y-4">
                            ...
                            <div className="flex gap-3 mt-6">
                                <button type="submit" className="flex-1 bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700 transition">إسناد المحكم</button>
                                ...
                                <div className="space-y-4">
                                    ...
                                    {aggregationData.details?.map((detail, idx) => (
                                        <div key={idx} className="p-5 border border-gray-100 rounded-2xl bg-gray-50/50">
                                            ...
                                            {detail.report_file_path && (
                                                <div className="mt-4 flex justify-end">
                                                    <a 
                                                        href={`/storage_file/${detail.report_file_path}`} 
                                                        target="_blank" 
                                                        rel="noopener noreferrer"
                                                        className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black border border-indigo-100 hover:bg-indigo-100 transition flex items-center gap-2 shadow-sm"
                                                    >
                                                        <span>📂</span>
                                                        عرض تقرير المحكم المرفق
                                                    </a>
                                                </div>
                                            )}
                                        </div>
