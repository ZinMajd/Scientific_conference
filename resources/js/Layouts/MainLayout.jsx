            ...
            <header style={{ background: 'linear-gradient(135deg, #001a2e 0%, #003153 60%, #004472 100%)' }}
                ...
                <div className="w-full px-4 lg:px-8 py-3 flex items-center justify-between min-h-[80px]">
                    ...
                    <div className="flex items-center gap-3 z-20 w-[20%] xl:w-[25%]">
                        <div className="w-12 h-12 lg:w-14 lg:h-14 bg-transparent rounded-none flex items-center justify-center overflow-hidden shrink-0">
                            <img src="/images/saba_logo.gif" alt={currentLang === 'en' ? 'Saba Region University' : 'جامعة إقليم سبأ'} className="w-full h-full object-contain notranslate" translate="no" />
                        </div>
                        <div className="hidden xl:block">
                            <h1 className="text-lg font-black text-white tracking-wide leading-tight whitespace-nowrap notranslate" translate="no">
                                {currentLang === 'en' ? 'Saba Region University' : 'جامعة إقليم سبأ'}
                            </h1>
                            <p className="text-[10px] text-teal-400 font-bold tracking-widest uppercase whitespace-nowrap notranslate" translate="no">
                    ...
                    <nav className="hidden lg:flex gap-4 xl:gap-6 items-center justify-center text-[15px] xl:text-[17px] font-bold whitespace-nowrap flex-1">
                        <Link to="/" className={`notranslate ${getLinkClass('/')}`} translate="no">{currentLang === 'en' ? 'Home' : 'الرئيسية'}</Link>
                        <Link to="/conferences" className={`notranslate ${getLinkClass('/conferences')}`} translate="no">{currentLang === 'en' ? 'Conferences' : 'المؤتمرات'}</Link>
                        <Link to="/about" className={`notranslate ${getLinkClass('/about')}`} translate="no">{currentLang === 'en' ? 'About' : 'عن النظام'}</Link>
                        <Link to="/faq" className={`notranslate ${getLinkClass('/faq')}`} translate="no">{currentLang === 'en' ? 'FAQ' : 'الأسئلة الشائعة'}</Link>
                        <Link to="/support" className={`notranslate ${getLinkClass('/support')}`} translate="no">{currentLang === 'en' ? 'Support' : 'الدعم'}</Link>
                        {user && (
                            <Link 
                                to={
                    ...
                    <div className="hidden md:flex items-center justify-end gap-4 xl:gap-6 text-[15px] xl:text-[17px] font-bold w-[20%] xl:w-[25%] z-20">
                        {!user ? (
                            <>
                                <Link to="/login" className={`notranslate ${getLinkClass('/login')}`} translate="no">
                                    {currentLang === 'en' ? 'Login' : 'تسجيل الدخول'}
                                </Link>
                                <Link to="/register" className={`notranslate ${getLinkClass('/register')}`} translate="no">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <span className="text-teal-400 font-bold text-base notranslate" translate="no">{user?.full_name || user?.name || (currentLang === 'en' ? 'User' : 'مستخدم')}</span>
                                <button
                                    onClick={handleLogout}
                                    className="px-4 py-2 text-red-300 font-bold hover:bg-red-500/10 border-2 border-red-400/20 hover:border-red-400/50 rounded-none transition-all notranslate"
                                    translate="no"
                                >
                ...
                {isMenuOpen && (
                    <div className="md:hidden border-t border-teal-400/20 p-4 flex flex-col gap-2"
                        ...
                        <Link to="/faq" className="p-3 text-white hover:text-teal-400 hover:bg-white/5 rounded-none transition notranslate" translate="no" onClick={closeMenu}>{currentLang === 'en' ? 'FAQ' : 'الأسئلة الشائعة'}</Link>
                        <Link to="/support" className="p-3 text-white hover:text-teal-400 hover:bg-white/5 rounded-none transition notranslate" translate="no" onClick={closeMenu}>{currentLang === 'en' ? 'Support' : 'الدعم'}</Link>
                        {!user && (
                            <Link to="/login" className="p-3 text-center border border-teal-400/40 text-teal-400 rounded-none notranslate" translate="no" onClick={closeMenu}>
                                {currentLang === 'en' ? 'Login' : 'تسجيل الدخول'}
                            </Link>
                        )}
                    </div>
            ...
            <footer style={{ background: 'linear-gradient(135deg, #001a2e 0%, #003153 100%)' }}
                ...
                <div className="container mx-auto px-4 grid md:grid-cols-3 gap-12 text-center md:text-right">
                    <div>
                        ...
                        <p className="text-sm leading-relaxed text-white/60 notranslate" translate="no">
                            {currentLang === 'en' 
                    ...
                    <div>
                        <h3 className="text-white font-bold text-lg mb-4 notranslate" translate="no">
                            {currentLang === 'en' ? 'Quick Links' : 'روابط سريعة'}
                        </h3>
                        <div className="w-12 h-0.5 mb-4 rounded-none" style={{ background: '#40E0D0' }}></div>
                        <ul className="space-y-3 text-sm">
                            <li><Link to="/conferences" className="hover:text-teal-400 transition-colors notranslate" translate="no">{currentLang === 'en' ? 'Available Conferences' : 'المؤتمرات المتاحة'}</Link></li>
                            <li><Link to="/support" className="hover:text-teal-400 transition-colors notranslate" translate="no">{currentLang === 'en' ? 'FAQ & Support' : 'الأسئلة الشائعة'}</Link></li>
                            <li><Link to="/login" className="hover:text-teal-400 transition-colors notranslate" translate="no">{currentLang === 'en' ? 'Researcher Registration' : 'تسجيل الباحثين'}</Link></li>
                        </ul>
                    </div>
                    <div>
                        <h3 className="text-white font-bold text-lg mb-4 notranslate" translate="no">
                ...
                <div className="text-center text-sm text-white/30 mt-10 pt-6 border-t border-teal-400/10 notranslate" translate="no">
