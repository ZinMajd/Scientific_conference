...
export default function ResearcherLayout() {
    ...
    const menuItems = [
        ...
        {
            title: 'المؤتمرات المسجل بها',
            icon: '📅',
            path: '/researcher/conferences',
            group: 'المؤتمرات',
            subItems: [
        ...
        {
            title: 'الأبحاث المحكمة',
            icon: '✅',
            path: '/researcher/reviewed',
            group: 'الأبحاث'
        },
        {
            title: 'سجل التحكيم',
            icon: '📜',
            path: '/researcher/reviews',
            group: 'الأبحاث'
        },
        {
            title: 'الشهادات',
            icon: '📜',
            path: '/researcher/certificates',
            group: 'الوثائق',
            subItems: [
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-row-reverse font-['Inter', 'Cairo', sans-serif]" dir="rtl">
            {/* Sidebar */}
            <aside className={`${isSidebarOpen ? 'w-72' : 'w-20'} bg-blue-950 transition-all duration-300 flex flex-col sticky top-0 h-screen shadow-2xl z-50`}>
                <div className="p-6 flex items-center justify-between border-b border-blue-900/50">
                    {isSidebarOpen && (
                        <span className="text-xl font-black text-white truncate">لوحة الباحث</span>
                    )}
                ...
                <div className="flex-1 overflow-y-auto py-6 px-4 space-y-8 custom-scrollbar">
                    {menuItems.map((item, idx) => (
                        <div key={idx}>
                            {isSidebarOpen && (
                                <h4 className="text-[10px] uppercase tracking-widest text-blue-400 font-bold mb-3 px-4">
                                    {item.group}
                                </h4>
                            )}
                            <Link 
                            ...
                            {isSidebarOpen && item.subItems && (
                                <div className="mt-2 mr-6 border-r border-blue-800 pr-4 space-y-1">
